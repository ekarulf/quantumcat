...
func (b *locoBackend) Close() error {
	b.closeOnce.Do(func() {
		...
		b.mu.Lock()
		clear(b.presharedKey[:])
		for k := range b.peerPSKs {
			b.peerPSKs[k] = PresharedKey{}
		}
		clear(b.peerPSKs)
		b.mu.Unlock()
	})
...
func (s *Server) Start() error {
	...
	lb.isServer = true
	lb.bootstrapClose = s.BootstrapClose
	lb.tunDevice = s.TUN
	lb.tunnelPeer = s.OnTunnelPeer
	if s.Bootstrap != nil {
		...
		go func() {
			...
			for {
				...
				case <-ticker.C:
					...
					if len(removed) > 0 && lb.nm != nil {
						nm := *lb.nm
						nm.Peers = nil
						for _, n := range lb.clients {
							nm.Peers = append(nm.Peers, n.View())
						}
						lb.nm = &nm
						lb.sys.MagicSock.Get().SetNetworkMap(nm.SelfNode, nm.Peers)
	...
	if s.TUN != nil {
		local := new(netipx.IPSetBuilder)
		local.AddPrefix(lb.addrPrefix)
		set, _ := local.IPSet()
		rules := []filter.Match{{IPProto: views.SliceOf([]ipproto.Proto{ipproto.TCP, ipproto.UDP, ipproto.ICMPv6}), Srcs: []netip.Prefix{netip.MustParsePrefix("fd7a:7163:6174::/48")}, Dsts: []filter.NetPortRange{{Net: lb.addrPrefix, Ports: allPorts}}}}
		sys.Engine.Get().SetFilter(filter.New(rules, nil, set, tailcatULASet(), nil, logf))
	}
...
func (b *locoBackend) peerConfig(k key.NodePublic) (_ wgcfg.PeerConfig, ok bool) {
	...
	if !b.serverPub.IsZero() {
		if b.authenticated && b.presharedKey.IsZero() {
...
func tcAddrForKey(k key.NodePublic) netip.Addr {
	var a [16]byte
	r := k.Raw32()
	// Use Quantumcat's ULA range fd7a:7163:6174::/48, filling the
...
func (c *Client) initLocked() error {
	...
	if c.Bootstrap != nil {
		lb.authenticated = true
		lb.presharedKey = PresharedKey{}
		c.bootstrapInbox = make(chan []byte, 16)
	}
	lb.logf = logf
	lb.dm = &tailcfg.DERPMap{}
	...
	lb.onDERPRecv = func(regionID tailcfg.DERPRegionID, src key.NodePublic, pkt []byte) bool {
		if c.Bootstrap != nil {
			if len(pkt) >= 4 && string(pkt[:4]) == "QCAT" {
				if src == lb.serverPub && len(pkt) <= 32768 {
					select {
					case c.bootstrapInbox <- append([]byte(nil), pkt...):
					default:
					}
				}
				return true
	...
	if c.TUN != nil {
		rules := []filter.Match{{IPProto: views.SliceOf([]ipproto.Proto{ipproto.TCP, ipproto.UDP, ipproto.ICMPv6}), Srcs: []netip.Prefix{pfxOf(tcAddrForKey(lb.serverPub))}, Dsts: []filter.NetPortRange{{Net: lb.addrPrefix, Ports: allPorts}}}}
		e.SetFilter(filter.New(rules, nil, local, tailcatULASet(), nil, logf))
	}
	dialer.UseNetstackForIP = func(ip netip.Addr) bool {
		_, ok := lb.peerByIP(ip)
		return ok
...
func (c *Client) ensureStarted(ctx context.Context) error {
	...
	if c.Bootstrap != nil {
		send := func(pkt []byte) error {
			_, err := c.lb.sys.MagicSock.Get().SendDERPPacketTo(c.lb.serverPub, c.lb.derpRegionID(), pkt)
			return err
		}
		...
		if psk.IsZero() {
			c.lb.Close()
			c.lb = nil
			return errors.New("bootstrap returned zero PSK")
		}
		c.lb.mu.Lock()
		c.lb.presharedKey = psk
		c.lb.mu.Unlock()
		c.upDone.Store(true)
		close(c.meowWait)
...
func (c *Client) Ping(ctx context.Context) (PingResult, error) {
	if c.Bootstrap != nil {
		...
		return PingResult{Latency: time.Since(start)}, err
	}
	var zero PingResult
